...
function MatchesView({ matches, predictions, profileId }) {
  ...
  const matchesByDate = filteredMatches.reduce((acc, match) => {
    const d = match.date || "Unscheduled";
    if (!acc[d]) acc[d] = [];
    acc[d].push(match);
    return acc;
  }, {});
    ...
    <div className="space-y-6 text-left">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        <div>
          ...
          <p className="text-xs text-slate-400">View and submit scores for all 104 matches</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs flex items-center gap-1 bg-slate-800 text-emerald-400 px-3 py-1.5 rounded-full border border-slate-700">
            <Clock className="w-3 h-3"/> Bahrain Time (GMT+3)
          </span>
        </div>
      </div>
      
      <div className="bg-slate-800/80 p-1.5 rounded-xl border border-slate-700 flex gap-1 max-w-md mr-auto">
        <button onClick={() => setFilterType('all')} className={`flex-1 text-center py-2 px-3 rounded-lg text-xs font-bold transition-all duration-200 ${filterType === 'all' ? 'bg-emerald-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}`}>Show All</button>
        <button onClick={() => setFilterType('today')} className={`flex-1 text-center py-2 px-3 rounded-lg text-xs font-bold transition-all duration-200 ${filterType === 'today' ? 'bg-emerald-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}`}>Today's Matches</button>
        <button onClick={() => setFilterType('upcoming')} className={`flex-1 text-center py-2 px-3 rounded-lg text-xs font-bold transition-all duration-200 ${filterType === 'upcoming' ? 'bg-emerald-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white hover:bg-slate-700/50'}`}>Hide Finished</button>
      </div>
      ...
      {sortedDates.length === 0 ? (
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-12 text-center text-slate-400 shadow-md">
          ...
          <p className="text-base font-bold text-white">No matches found for this filter</p>
          <p className="text-xs text-slate-500 mt-1">Click on "Show All" to browse the rest of the tournament.</p>
        </div>
      ) : (
      ...
      <div className="mt-4 pt-4 border-t border-slate-700/50 flex justify-between items-center h-10">
        {isCompleted ? (
          <div className="w-full flex justify-between items-center">
            <span className="text-sm font-medium text-slate-400">Result: <span className="text-white font-bold">{match.actualA} - {match.actualB}</span></span>
            {earnedPoints !== null && (
          ...
          <div className="w-full flex justify-between items-center">
             ...
             {!isLockedForUser && (
               <button 
                 ...
                 className={`flex items-center gap-1 px-4 py-2 rounded-lg text-sm font-bold transition ${saved ? 'bg-emerald-500/20 text-emerald-400' : hasChanges && scoreA !== '' && scoreB !== '' ? 'bg-emerald-500 text-slate-900 shadow-md' : 'bg-slate-700 text-slate-400 cursor-not-allowed'}`}
               >
